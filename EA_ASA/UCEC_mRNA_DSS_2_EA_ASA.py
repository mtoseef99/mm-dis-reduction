...
def run_cv(cancer_type, feature_type, target, years=3):
    ...
    dataset_asian = get_one_race(dataset, 'ASIAN')
    dataset_asian = get_n_years(dataset_asian, years)
    ...
    parametrs_one_race_white = {'fold': 4, 'k': k, 'val_size': 0.0, 'batch_size': 20,
                   'learning_rate': 0.01, 'lr_decay': 0.0, 'dropout': 0.5,
                   'L1_reg': 0.001, 'L2_reg': 0.001, 'hiddenLayers': [128, 64]}
    parametrs_one_race_asian = {'fold': 4, 'k': k, 'val_size': 0.0, 'batch_size': 4,
                   'learning_rate': 0.01, 'lr_decay': 0.0, 'dropout': 0.5,
                   'L1_reg': 0.001, 'L2_reg': 0.001, 'hiddenLayers': [128, 64]}
    ...
    for i in range(20):
        ...
        df_asian = run_one_race_cv(seed, dataset_asian, **parametrs_one_race_native)
        df_asian = df_asian.rename(columns={"Auc": "Native_ind"})
